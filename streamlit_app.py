...
def load_api_key_from_secrets():
    """Streamlit Cloud exposes secrets through st.secrets, not the environment.

    The retrieval and answer modules deliberately know nothing about Streamlit, so
    that the unit tests and the evaluation run without it. This bridges the two.
    """
    for name in ("GEMINI_API_KEY", "GEMINI_CHAT_MODEL"):
...
def describe_failure(error):
    """One honest sentence per cause, so nobody is told to fix the wrong thing."""
    text = f"{type(error).__name__}: {error}"
    if "GEMINI_API_KEY" in text:
        return ("This deployment has no API key configured, so I can't answer yet. "
                "If you manage this app, add GEMINI_API_KEY to its secrets.")
    if re.search(r"stale|sha-?256|does not match", text, re.IGNORECASE):
        return ("The evidence index doesn't match the source corpus. I've stopped "
                "rather than answer from a mismatched index.")
    if re.search(r"429|quota|rate.?limit|RESOURCE_EXHAUSTED", text, re.IGNORECASE):
        return ("This demo has used up its free request allowance for now. "
                "Please try again a little later.")
    if re.search(r"timeout|timed out|connection|network|unavailable|50\d", text,
                 re.IGNORECASE):
        return ("I couldn't reach the language model just now. Please try your "
                "question again in a moment.")
    return ("Something went wrong while preparing that answer. Please try again — "
            "the app's logs will have the details.")
...
with st.container(key="app_panel", gap="small"):
    ...
    st.title(APP_TITLE)
    st.caption("AI assistant grounded in Lime Green technical guidance.")
...
if st.session_state.pending_question:
    ...
    try:
        ...
        # The real cause goes to the server log; the user gets the one line that
        # tells them whether to wait, retry, or report it.
        traceback.print_exc()
