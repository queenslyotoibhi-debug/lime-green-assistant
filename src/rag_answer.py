...
from gemini_embeddings import get_api_key
from governed_search import governed_search
from query_processor import process_query


DEFAULT_MODEL = "gemini-3.5-flash-lite"
MODEL = os.environ.get("GEMINI_CHAT_MODEL", "").strip() or DEFAULT_MODEL
TEMPERATURE = 0
...
# Free-tier capacity is shared, so a flash model intermittently returns 503 under
# load. That is a wait-and-retry condition, not a failed answer.
CHAT_MAX_ATTEMPTS = 4
CHAT_RETRY_SECONDS = 8.0
RETRYABLE_MARKERS = ("503", "UNAVAILABLE", "429", "RESOURCE_EXHAUSTED", "high demand")
# A per-day quota does not refill inside any sensible backoff window, so retrying
# it just delays the error by a minute. Per-minute limits do recover.
EXHAUSTED_MARKERS = ("PerDay", "RequestsPerDay", "per day")


def invoke_with_retry(chain, payload, progress=None):
    delay = CHAT_RETRY_SECONDS
    ...
    for attempt in range(1, CHAT_MAX_ATTEMPTS + 1):
        try:
            ...
            retryable = (
                any(marker in text for marker in RETRYABLE_MARKERS)
                and not any(marker in text for marker in EXHAUSTED_MARKERS)
            )

            if not retryable or attempt == CHAT_MAX_ATTEMPTS:
                raise

            if progress:
                progress(f"  model unavailable, retrying in {delay:.0f}s "
                         f"(attempt {attempt}/{CHAT_MAX_ATTEMPTS - 1})")
            time.sleep(delay)
            delay = min(delay * 2, 45.0)


def extract_response_text(response):
    """Return the visible answer text, whatever shape the provider used.

    Gemini 3.x returns content as a list of typed blocks rather than a string, so
    reading `.content` directly yields a list. LangChain exposes the concatenated
    visible text as `.text` — a property in langchain-core 1.x, a method in some
    earlier versions — with the block walk kept as a last resort.
    """
...
CITATION_GROUP_PATTERN = re.compile(r"\[([^\]]*?S\s*\d+[^\]]*?)\]", re.IGNORECASE)
SOURCE_NUMBER_PATTERN = re.compile(r"S\s*(\d+)", re.IGNORECASE)


def find_source_numbers(text):
    """Every source number cited, in order, including grouped forms.

    The prompt asks for [S1], [S2]; models also write [S1, S2] and [S1, S2, S3].
    Resolving citations is meant to be deterministic code rather than a formatting
    promise from the model, so accept both rather than silently dropping a group
    and under-reporting which sources an answer actually used.
    """
    numbers = []
...
def extract_cited_source_numbers(answer):
    return sorted(set(find_source_numbers(answer)))
