...
def load_index():
    ...
    manifest_task_type = manifest.get("embedding_task_type")

    if manifest_task_type != DOCUMENT_TASK_TYPE:
        raise ValueError(
            "The index was built with a different embedding task type: "
            f"expected {DOCUMENT_TASK_TYPE}, found {manifest_task_type}. "
...
def embed_query(query):
    [values] = embed_texts([query], QUERY_TASK_TYPE)
    query_vector = np.asarray(values, dtype=np.float32)
